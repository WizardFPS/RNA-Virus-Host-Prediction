...
PATH_SAMPLE_IDS = "/home/pereligin/host_prediction/sample_ids/genomes_fragments_connected/"+taxa_level
...
PATH_CLASSIFICATORS = "/home/pereligin/host_prediction/classificators/genomes_fragments_connected/"+taxa_level+f"iters_{SAMPLE_TYPE}/"

scoring="f1_weighted"

classificators_sole_genomes = pickle.load(open(PATH_CLASSIFICATORS+f"classificators_comb_{SAMPLE_TYPE}.pkl", "rb"))

def params_changer(params, name, value):
    params[name] = value
    return params


rf_params = {key:params_changer(value[0].get_params(), "n_jobs", -1) for key, value in classificators_sole_genomes.items()}
lgbm_params = {key:params_changer(value[1].get_params(), "n_jobs", -1) for key, value in classificators_sole_genomes.items()}
xgb_params = {key:params_changer(value[2].get_params(), "n_jobs", -1) for key, value in classificators_sole_genomes.items()}
svc_params = {key:[params_changer(value[3][i][1].best_params_, "probability", True) for i in range(3)] for key, value in classificators_sole_genomes.items()}
...
def calc_models(models, X_train, y_train, X_test, y_test, indices_test, indices_train, y_class_h, list_y, classes, y_int, params):
    ...
    for el in models:
        ...
        if el == 'rf':
            inside_params = params[0]

            model = RandomForestClassifier(**inside_params).fit(X_train, y_train)
            classificators.append(model)
            y_pred = model.predict(X_test)
            cl_reps.append(classification_report(y_test, y_pred, output_dict = True, zero_division=zero_div))
            #print(el, classification_report(y_test, y_pred, zero_division=zero_div))

        if el == 'lgbm':
            inside_params = params[1]
            
            model = LGBMClassifier(**inside_params).fit(X_train, y_train)
            classificators.append(model)
            y_pred = model.predict(X_test)
            cl_reps.append(classification_report(y_test, y_pred, output_dict = True, zero_division=zero_div))
            #print(el, classification_report(y_test, y_pred, zero_division=zero_div))
        ...
        if el == 'xgb':
            inside_params = params[2]
            le = LabelEncoder()
            y_train, y_test = le.fit_transform(y_train), le.fit_transform(y_test)

            model = xgboost.XGBClassifier(**inside_params).fit(X_train, y_train)
            classificators.append(model)
            y_pred = model.predict(X_test)
            cl_reps.append(classification_report(y_test, y_pred, output_dict = True, target_names = classes, zero_division=zero_div))
            #print(el, classification_report(y_test, y_pred, target_names = classes, zero_division=zero_div))
        ...
        if el == 'svc':
            ...
            for y_class, class_name in tqdm(zip(list_y, classes)):
                ...
                svc_classificators.append(model)
                y_pred = model.predict(X_test)
                y_proba = np.vstack((y_proba, model.predict_proba(X_test)[:,1]))
                print(class_name)

                #print(classification_report(y_test, y_pred, target_names = ['Others', class_name], zero_division=zero_div))
                svc_classes.append(classification_report(y_test, y_pred, output_dict=True, target_names = ['Others', class_name], zero_division=zero_div))
            ...
            cl_reps.append(classification_report(y_int[indices_test], y_pred_all, output_dict = True, target_names = classes, zero_division=zero_div))
...
def training(models, features_sets, y_class_h, full_or_ranged, subseq_len = ""):
    ...
    if len(features_sets[0]) > 1: # если длина 1 призака > 1, то это all_sets (комбинации признаков)
        ...
        for fset in features_sets: # берём комбинацию признаков (fset)
            fset_index = features_sets.index(fset)
            params = [x[fun_all_sets_names[fset_index]] for x in [rf_params, lgbm_params, xgb_params, svc_params]]
            ...
            out = calc_models(models, X_train, y_train, X_test, y_test, indices_test, indices_train, y_class_h, list_y, classes, y_int, params) # Обучаем модели
        ...
        pickle.dump(all_clfs, open(fun_PATH_REPORTS + "reports_comb_" + filename_word + "_" + iteration + ".pkl", "wb"))
        pickle.dump(svc_classes_clfs, open(fun_PATH_REPORTS + "reports_svc_classes_comb_" + filename_word + "_" + iteration + ".pkl", "wb"))
        pickle.dump(all_classificators, open(fun_PATH_CLASSIFICATORS + "classificators_comb_" + filename_word + "_" + iteration + ".pkl", "wb"))
    
    if len(features_sets[0]) == 1: # если длина первого призака = 1, то это sole_sets (одиночные признаки)
        
        for fset in features_sets:
            fset_index = features_sets.index(fset)
            params = [x[fun_sole_sets_names[fset_index]] for x in [rf_params, lgbm_params, xgb_params, svc_params]]
            #params = [x["R"+fset[0][1:]] for x in [rf_params, lgbm_params, xgb_params, svc_params]]
        ...
        pickle.dump(all_clfs, open(fun_PATH_REPORTS + "reports_sole_" + filename_word + "_" + iteration + ".pkl", "wb"))
        pickle.dump(svc_classes_clfs, open(fun_PATH_REPORTS + "reports_svc_classes_sole_" + filename_word + "_" + iteration + ".pkl", "wb"))
        pickle.dump(all_classificators, open(fun_PATH_CLASSIFICATORS + "classificators_sole_" + filename_word + "_" + iteration + ".pkl", "wb"))
...
train_val_test_ids_outside = pickle.load(open(PATH_SAMPLE_IDS + f"train_val_test_{SAMPLE_TYPE}_" + taxa_level[:-1] + ".pkl", "rb"))
for it in range(10):
    iteration = str(it)
    train_val_test_ids = train_val_test_ids_outside[it]
    #training(models, sole_sets, "host", "full")
    training(models, all_sets, "host", "ranged", SAMPLE_TYPE)
